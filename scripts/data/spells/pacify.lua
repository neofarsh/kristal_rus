...
function spell:init()
    ...
    self.effect = "Пощадит\nУСТАВЩЕГО"
    -- Menu description
    self.description = "ЩАДИТ уставшего врага, усыпляя его."
...
function spell:getCastMessage(user, target)
    ...
    elseif target.mercy < 100 then
        return message.."\n[wait:0.25s]* Но враг не был [color:blue]УСТАВШИМ[color:reset]..."
    else
        return message.."\n[wait:0.25s]* Но враг не был [color:blue]УСТАВШИМ[color:reset]... попробуйте\n[color:yellow]ПОЩАДУ[color:reset]!"
    end
end
